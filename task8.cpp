...
int main() {
    ...
    for (size_t chunk_size = 0; chunk_size < MAX_CHUNK_SIZE; chunk_size += CHUNK_STEP) {
        ...
        printf("\tOption %s, time: %.4f msec\n", TOSTR(STATIC),
               1000 * check_better_time(better_time, parallel_for_schedule_static(chunk_size), better_opt, STATIC));
        printf("\tOption %s, time: %.4f msec\n", TOSTR(DYNAMIC),
               1000 * check_better_time(better_time, parallel_for_schedule_dynamic(chunk_size), better_opt, DYNAMIC));
        printf("\tOption %s, time: %.4f msec\n", TOSTR(GUIDED),
               1000 * check_better_time(better_time, parallel_for_schedule_guided(chunk_size), better_opt, GUIDED));
        printf("\tOption %s, time: %.4f msec\n", TOSTR(RUNTIME),
               1000 * check_better_time(better_time, parallel_for_schedule_runtime(), better_opt, RUNTIME));
        printf("BETTER OPTION %d, BETTER TIME %.4f msec, CHUNK_SIZE = %d\n\n", better_opt, 1000 * better_time,
               chunk_size);
        better_opt_cout[better_opt - 1]++;
    }
    printf("BETTER Options count:\nSTATIC: %d\nDYNAMIC: %d\nGUIDED: %d\nRUNTIME: %d\n", better_opt_cout[STATIC - 1],
           better_opt_cout[DYNAMIC - 1], better_opt_cout[GUIDED - 1], better_opt_cout[RUNTIME - 1]);
